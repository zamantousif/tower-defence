...
namespace td
{
    Tower::Tower(sf::Vector2<float> position, sf::CircleShape hitbox, sf::Texture sprite, float rotation_angle, unsigned int attack_speed, float range, unsigned int level) : 
        Object(position, hitbox, sprite, rotation_angle), attack_speed_(attack_speed), range_(range), level_(level) {}
